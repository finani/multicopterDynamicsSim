 ...
 * @param motorRotationalInertia Motors rotational mass moment of inertia
 * (including propeller)
 ...
 * @param momentProcessNoiseAutoCorrelation Vehicle dynamics stochastic moment
 * process noise auto correlation
 * @param forceProcessNoiseAutoCorrelation Vehicle dynamics stochastic force
 * process noise auto correlation
 * @param gravity Gravity vector in world-fixed reference frame
 */
MulticopterDynamicsSim::MulticopterDynamicsSim(
    int numCopter, double thrustCoefficient, double torqueCoefficient,
    double minMotorSpeed, double maxMotorSpeed, double motorTimeConstant,
    double motorRotationalInertia, double vehicleMass,
    const Eigen::Matrix3d& vehicleInertia,
    const Eigen::Matrix3d& aeroMomentCoefficient, double dragCoefficient,
    double momentProcessNoiseAutoCorrelation,
    double forceProcessNoiseAutoCorrelation, const Eigen::Vector3d& gravity)
 ...
 * process noise auto correlation
 */
void MulticopterDynamicsSim::setVehicleProperties(
    double vehicleMass, const Eigen::Matrix3d& vehicleInertia,
    const Eigen::Matrix3d& aeroMomentCoefficient, double dragCoefficient,
    double momentProcessNoiseAutoCorrelation,
 ...
 *
 * @param motorFrame Motor orientation and position with regard to body-fixed
 reference frame
 * @param motorDirection Motor rotation direction
 *         +1 if positive motor speed corresponds to positive moment around the
 motor frame z-axis -1 if positive motor speed corresponds to negative moment
 around the motor frame z-axis i.e. -1 indicates a positive motor speed
 corresponds to a positive rotation rate around the motor z-axis
 ...
 * @param motorIndex Motor index number
 */
void MulticopterDynamicsSim::setMotorProperties(
    double thrustCoefficient, double torqueCoefficient,
    double motorTimeConstant, double minMotorSpeed, double maxMotorSpeed,
    double rotationalInertia, int motorIndex) {
...
void MulticopterDynamicsSim::setMotorProperties(double thrustCoefficient,
  ...
  for (int motorIndex = 0; motorIndex < numCopter_; motorIndex++) {
    setMotorProperties(thrustCoefficient, torqueCoefficient, motorTimeConstant,
                       minMotorSpeed, maxMotorSpeed, rotationalInertia,
                       motorIndex);
  }
}
 ...
 */
void MulticopterDynamicsSim::setVehiclePosition(
 ...
 */
void MulticopterDynamicsSim::setVehicleState(
    const Eigen::Vector3d& position, const Eigen::Vector3d& velocity,
    const Eigen::Vector3d& angularVelocity, const Eigen::Quaterniond& attitude,
...
Eigen::Vector3d MulticopterDynamicsSim::getVehicleSpecificForce(void) {
  Eigen::Vector3d specificForce =
      (getThrust(motorSpeed_) +
       attitude_.inverse().toRotationMatrix().transpose() *
           (getDragForce(velocity_) + stochForce_)) /
      vehicleMass_;
 ...
 */
Eigen::Vector3d MulticopterDynamicsSim::getThrust(
    const std::vector<double>& motorSpeed) {
  Eigen::Vector3d thrust = Eigen::Vector3d::Zero();

  for (int indx = 0; indx < numCopter_; indx++) {
    Eigen::Vector3d motorThrust(0., 0.,
                                fabs(motorSpeed.at(indx)) *
                                    motorSpeed.at(indx) *
 ...
 */
Eigen::Vector3d MulticopterDynamicsSim::getControlMoment(
    const std::vector<double>& motorSpeed,
  ...
  for (int indx = 0; indx < numCopter_; indx++) {
    ...
    Eigen::Vector3d motorThrust(0., 0.,
                                fabs(motorSpeed.at(indx)) *
                                    motorSpeed.at(indx) *
                                    thrustCoefficient_.at(indx));
    controlMoment += motorFrame_.at(indx).translation().cross(
        motorFrame_.at(indx).linear() * motorThrust);

    // Moment due to torque
    Eigen::Vector3d motorTorque(
        0., 0.,
        motorDirection_.at(indx) * fabs(motorSpeed.at(indx)) *
            motorSpeed.at(indx) * torqueCoefficient_.at(indx));
    motorTorque(2) += motorDirection_.at(indx) *
                      motorRotationalInertia_.at(indx) *
                      motorAcceleration.at(indx);
 ...
 */
Eigen::Vector3d MulticopterDynamicsSim::getAeroMoment(
 ...
 */
Eigen::Vector3d MulticopterDynamicsSim::getDragForce(
...
void MulticopterDynamicsSim::getIMUMeasurement(Eigen::Vector3d& accOutput,
  ...
  imu_.getMeasurement(accOutput, gyroOutput, getVehicleSpecificForce(),
                      angularVelocity_);
}
 ...
 */
void MulticopterDynamicsSim::proceedState_ExplicitEuler(
    double dt_secs, const std::vector<double>& motorSpeedCommand) {
  std::vector<double> motorSpeedCommandBounded(numCopter_);
  vectorBoundOp(motorSpeedCommand, motorSpeedCommandBounded, minMotorSpeed_,
                maxMotorSpeed_);
  ...
  stochMoment << sqrt(momentProcessNoiseAutoCorrelation_ / dt_secs) *
                     standardNormalDistribution_(randomNumberGenerator_),
      sqrt(momentProcessNoiseAutoCorrelation_ / dt_secs) *
          standardNormalDistribution_(randomNumberGenerator_),
      sqrt(momentProcessNoiseAutoCorrelation_ / dt_secs) *
          standardNormalDistribution_(randomNumberGenerator_);
  ...
  Eigen::Vector3d velocityDer =
      getVelocityDerivative(attitude, stochForce_, velocity, motorSpeed);
  Eigen::Vector4d attitudeDer =
      getAttitudeDerivative(attitude, angularVelocity);
  Eigen::Vector3d angularVelocityDer = getAngularVelocityDerivative(
      motorSpeed, motorSpeedDer, angularVelocity, stochMoment);
  ...
  stochForce_ << sqrt(forceProcessNoiseAutoCorrelation_) *
                     standardNormalDistribution_(randomNumberGenerator_),
 ...
 * @param motorSpeedCommand Motor speed commands
 */
void MulticopterDynamicsSim::proceedState_RK4(
    double dt_secs, const std::vector<double>& motorSpeedCommand) {
  std::vector<double> motorSpeedCommandBounded(numCopter_);
  vectorBoundOp(motorSpeedCommand, motorSpeedCommandBounded, minMotorSpeed_,
                maxMotorSpeed_);
  ...
  Eigen::Vector3d velocityDer =
      dt_secs *
      getVelocityDerivative(attitude_, stochForce_, velocity_, motorSpeed_);
  Eigen::Vector4d attitudeDer =
      dt_secs * getAttitudeDerivative(attitude_, angularVelocity_);
  Eigen::Vector3d angularVelocityDer =
      dt_secs * getAngularVelocityDerivative(motorSpeed_, motorSpeedDer,
                                             angularVelocity_, stochMoment);
  ...
  vectorAffineOp(motorSpeed_, motorSpeedDer, motorSpeedIntermediate,
                 0.5);  // x + 0.5*(k1)
  vectorBoundOp(motorSpeedIntermediate, motorSpeedIntermediate, minMotorSpeed_,
                maxMotorSpeed_);
  attitudeIntermediate.coeffs() = attitude_.coeffs() + attitudeDer * 0.5;
  attitudeIntermediate.normalize();

  getMotorSpeedDerivative(motorSpeedDer, motorSpeedIntermediate,
                          motorSpeedCommandBounded);
  positionDer = dt_secs * (velocity_ + 0.5 * velocityDer);
  velocityDer =
      dt_secs * getVelocityDerivative(attitudeIntermediate, stochForce_,
                                      (velocity_ + 0.5 * velocityDer),
                                      motorSpeedIntermediate);
  attitudeDer = dt_secs * getAttitudeDerivative(
                              attitudeIntermediate,
                              (angularVelocity_ + 0.5 * angularVelocityDer));
  angularVelocityDer =
      dt_secs * getAngularVelocityDerivative(
                    motorSpeedIntermediate, motorSpeedDer,
                    (angularVelocity_ + 0.5 * angularVelocityDer), stochMoment);
  vectorScalarProd(motorSpeedDer, motorSpeedDer, dt_secs);
  ...
  velocityDer =
      dt_secs * getVelocityDerivative(attitudeIntermediate, stochForce_,
                                      ...
                                      motorSpeedIntermediate);
  attitudeDer =
      dt_secs * getAttitudeDerivative(attitudeIntermediate,
                                      (angularVelocity_ + angularVelocityDer));
  angularVelocityDer =
      dt_secs * getAngularVelocityDerivative(
                    motorSpeedIntermediate, motorSpeedDer,
                    (angularVelocity_ + angularVelocityDer), stochMoment);
  vectorScalarProd(motorSpeedDer, motorSpeedDer, dt_secs);
...
void MulticopterDynamicsSim::vectorAffineOp(const std::vector<double>& vec1,
  ...
  std::transform(vec1.begin(), vec1.end(), vec2.begin(), vec3.begin(),
                 [val](const double& vec1val, const double& vec2val) -> double {
                   return (vec1val + val * vec2val);
                 });
}

/**
 * @brief Element-wise vector bound operation: vec2 = max(minvalue,
 * min(maxvalue, vec1))
 *
...
void MulticopterDynamicsSim::vectorBoundOp(const std::vector<double>& vec1,
  ...
  std::transform(vec1.begin(), vec1.end(), maxvec.begin(), vec2.begin(),
                 [](const double& vec1val, const double& maxvalue) -> double {
                   return fmin(vec1val, maxvalue);
                 });
  std::transform(vec2.begin(), vec2.end(), minvec.begin(), vec2.begin(),
                 [](const double& vec2val, const double& minvalue) -> double {
...
void MulticopterDynamicsSim::vectorScalarProd(const std::vector<double>& vec1,
  ...
  std::transform(vec1.begin(), vec1.end(), vec2.begin(),
                 [val](const double& vec1val) { return vec1val * val; });
}
 ...
 */
void MulticopterDynamicsSim::getMotorSpeedDerivative(
    std::vector<double>& motorSpeedDer, const std::vector<double>& motorSpeed,
    const std::vector<double>& motorSpeedCommand) {
  for (int indx = 0; indx < numCopter_; indx++) {
    motorSpeedDer.at(indx) =
        (motorSpeedCommand.at(indx) - motorSpeed.at(indx)) /
 ...
 */
Eigen::Vector3d MulticopterDynamicsSim::getVelocityDerivative(
    const Eigen::Quaterniond& attitude, const Eigen::Vector3d& stochForce,
    const Eigen::Vector3d& velocity, const std::vector<double>& motorSpeed) {
  return (gravity_ +
          (attitude.toRotationMatrix().transpose() * getThrust(motorSpeed) +
           getDragForce(velocity) + stochForce) /
              vehicleMass_);
}
 ...
 *
 * @param attitude Vehicle attitude
 * @param angularVelocity Vehicle angular velocity in vehicle-fixed reference
 * frame
 * @return Eigen::Vector4d Attitude derivative
 */
Eigen::Vector4d MulticopterDynamicsSim::getAttitudeDerivative(
 ...
 */
Eigen::Vector3d MulticopterDynamicsSim::getAngularVelocityDerivative(
    const std::vector<double>& motorSpeed,
    const std::vector<double>& motorAcceleration,
    const Eigen::Vector3d& angularVelocity,
  ...
  for (int indx = 0; indx < numCopter_; indx++) {
    ...
    motorAngularMomentum(2) = -1 * motorDirection_.at(indx) *
                              motorRotationalInertia_.at(indx) *
                              motorSpeed.at(indx);
  ...
  return (vehicleInertia_.inverse() *
          (getControlMoment(motorSpeed, motorAcceleration) +
           getAeroMoment(angularVelocity) + stochMoment -
