...
class MulticopterDynamicsSim {
 public:
  MulticopterDynamicsSim(int numCopter, double thrustCoefficient,
                         double torqueCoefficient, double minMotorSpeed,
                         double maxMotorSpeed, double motorTimeConstant,
  ...
  void setVehicleProperties(double vehicleMass,
                            const Eigen::Matrix3d& vehicleInertia,
                            const Eigen::Matrix3d& aeroMomentCoefficient,
                            double dragCoefficient,
                            double momentProcessNoiseAutoCorrelation,
  ...
  void setRandomSeed(const unsigned multicopterSeed, const unsigned imuSeed);
  void setMotorFrame(const Eigen::Isometry3d& motorFrame, int motorDirection,
                     int motorIndex);
  void setMotorProperties(double thrustCoefficient, double torqueCoefficient,
                          double motorTimeConstant, double minMotorSpeed,
                          double maxMotorSpeed, double rotationalInertia,
                          int motorIndex);
  void setMotorProperties(double thrustCoefficient, double torqueCoefficient,
                          double motorTimeConstant, double minMotorSpeed,
                          double maxMotorSpeed, double rotationalInertia);
  ...
  void setVehiclePosition(const Eigen::Vector3d& position,
  ...
  void getVehicleState(Eigen::Vector3d& position, Eigen::Vector3d& velocity,
  ...
  void proceedState_ExplicitEuler(double dt_secs,
                                  const std::vector<double>& motorSpeedCommand);
  void proceedState_RK4(double dt_secs,
                        const std::vector<double>& motorSpeedCommand);

  void getIMUMeasurement(Eigen::Vector3d& accOutput,
 ...
 private:
  ...
  /* +1 if positive motor speed corresponds to positive moment around the motor
     frame z-axis -1 if positive motor speed corresponds to negative moment
  ...
  */
  ...
  // Default reference frame is NED, but can be set by changing gravity
  // direction
  /// @name Gravity vector
  ...
  Eigen::Vector3d getControlMoment(
      const std::vector<double>& motorSpeed,
  ...
  void getMotorSpeedDerivative(std::vector<double>& motorSpeedDer,
                               const std::vector<double>& motorSpeed,
                               const std::vector<double>& motorSpeedCommand);
  Eigen::Vector3d getVelocityDerivative(const Eigen::Quaterniond& attitude,
                                        const Eigen::Vector3d& stochForce,
                                        const Eigen::Vector3d& velocity,
                                        const std::vector<double>& motorSpeed);
  Eigen::Vector3d getAngularVelocityDerivative(
      const std::vector<double>& motorSpeed,
      const std::vector<double>& motorAcceleration,
      const Eigen::Vector3d& angularVelocity,
      const Eigen::Vector3d& stochMoment);
  Eigen::Vector4d getAttitudeDerivative(const Eigen::Quaterniond& attitude,
                                        const Eigen::Vector3d& angularVelocity);
  void vectorAffineOp(const std::vector<double>& vec1,
                      const std::vector<double>& vec2,
                      std::vector<double>& vec3, double val);
  void vectorScalarProd(const std::vector<double>& vec1,
                        std::vector<double>& vec2, double val);
