 ...
 * @param accBiasProcessNoiseAutoCorrelation Accelerometer bias process noise
 * auto correlation
 * @param gyroBiasProcessNoiseAutoCorrelation Gyroscope bias process noise auto
 * correlation
 */
 ...
 * @param gyroBias Gyroscope bias value
 */
void inertialMeasurementSim::getBias(Eigen::Vector3d& accBias,
...
void inertialMeasurementSim::setBias(
    ...
    double accBiasProcessNoiseAutoCorrelation,
    double gyroBiasProcessNoiseAutoCorrelation) {
  accBias_ << sqrt(accBiasVariance) *
                  standardNormalDistribution_(randomNumberGenerator_),
      sqrt(accBiasVariance) *
          standardNormalDistribution_(randomNumberGenerator_),
      sqrt(accBiasVariance) *
          standardNormalDistribution_(randomNumberGenerator_);

  gyroBias_ << sqrt(gyroBiasVariance) *
                   standardNormalDistribution_(randomNumberGenerator_),
      sqrt(gyroBiasVariance) *
          standardNormalDistribution_(randomNumberGenerator_),
      sqrt(gyroBiasVariance) *
          standardNormalDistribution_(randomNumberGenerator_);
...
void inertialMeasurementSim::setOrientation(
    const Eigen::Quaterniond& imuOrient) {
...
void inertialMeasurementSim::getMeasurement(
    Eigen::Vector3d& accOutput, Eigen::Vector3d& gyroOutput,
    const Eigen::Vector3d specificForce,
    const Eigen::Vector3d angularVelocity) {
  Eigen::Vector3d accNoise;

  accNoise << sqrt(accMeasNoiseVariance_) *
                  standardNormalDistribution_(randomNumberGenerator_),
      sqrt(accMeasNoiseVariance_) *
          standardNormalDistribution_(randomNumberGenerator_),
      sqrt(accMeasNoiseVariance_) *
          standardNormalDistribution_(randomNumberGenerator_);
  ...
  gyroNoise << sqrt(gyroMeasNoiseVariance_) *
                   standardNormalDistribution_(randomNumberGenerator_),
      sqrt(gyroMeasNoiseVariance_) *
          standardNormalDistribution_(randomNumberGenerator_),
      sqrt(gyroMeasNoiseVariance_) *
          standardNormalDistribution_(randomNumberGenerator_);
...
void inertialMeasurementSim::proceedBiasDynamics(double dt_secs) {
  ...
  accBiasDerivative << sqrt(accBiasProcessNoiseAutoCorrelation_ / dt_secs) *
                           standardNormalDistribution_(randomNumberGenerator_),
      sqrt(accBiasProcessNoiseAutoCorrelation_ / dt_secs) *
          standardNormalDistribution_(randomNumberGenerator_),
      sqrt(accBiasProcessNoiseAutoCorrelation_ / dt_secs) *
          standardNormalDistribution_(randomNumberGenerator_);
  ...
  gyroBiasDerivative << sqrt(gyroBiasProcessNoiseAutoCorrelation_ / dt_secs) *
                            standardNormalDistribution_(randomNumberGenerator_),
      sqrt(gyroBiasProcessNoiseAutoCorrelation_ / dt_secs) *
          standardNormalDistribution_(randomNumberGenerator_),
      sqrt(gyroBiasProcessNoiseAutoCorrelation_ / dt_secs) *
          standardNormalDistribution_(randomNumberGenerator_);
